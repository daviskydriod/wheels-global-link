...
function ComparePage() {
  ...
  return <>
    ...
    <section className="section-pad"><div className="container-shell"><Link to="/cars" className="mb-8 inline-flex items-center gap-2 text-sm font-bold text-primary"><ArrowLeft className="h-4 w-4"/>Back to cars</Link>{selected.length < 2 ? <EmptyCompare /> : <div className="overflow-x-auto border-l border-t border-border"><table className="w-full min-w-[640px] border-collapse text-left"><thead><tr><th className="w-40 border-b border-r border-border bg-secondary p-4 text-xs font-bold uppercase">Specification</th>{selected.map((vehicle) => <th key={vehicle.slug} className="border-b border-r border-border p-4 align-top"><img src={vehicle.image} alt={`${vehicle.year} ${vehicle.brand} ${vehicle.model}`} className="mb-4 aspect-[16/10] w-full min-w-32 object-cover"/><span className="block text-xs font-bold uppercase text-primary">{vehicle.brand}</span><span className="mt-1 block text-xl font-bold uppercase">{vehicle.model}</span></th>)}</tr></thead><tbody>{rows.map(([label, getValue]) => <tr key={label}><th className="border-b border-r border-border bg-secondary p-4 text-xs font-bold uppercase text-muted-foreground">{label}</th>{selected.map((vehicle) => <td key={`${vehicle.slug}-${label}`} className="border-b border-r border-border p-4 text-sm font-semibold">{getValue(vehicle)}</td>)}</tr>)}</tbody></table></div>}</div></section>
  </>;
}
