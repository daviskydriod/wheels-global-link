...
const navItems = [
  ...
  { label: "Inquiries", icon: ClipboardList, count: 18 },
...
function AdminPage() {
  ...
  useEffect(() => {
    ...
    apiRequest<Summary>("/admin/summary")
      .then(setSummary)
      .catch(() => setSummary(previewSummary))
    ...
    <div className="min-h-screen bg-[#f5f7fb] text-foreground">
      <div className="flex min-h-screen">
        ...
        <div className="flex min-w-0 flex-1 flex-col lg:pl-64">
          ...
          {mobileMenu && (
            <div className="fixed inset-0 z-20 bg-[#101b33] p-5 pt-20 text-white lg:hidden">
              <AdminBrand compact />
          ...
          <nav className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-5 border-t border-slate-200 bg-white/95 pb-[env(safe-area-inset-bottom)] shadow-[0_-8px_30px_rgba(15,23,42,.08)] backdrop-blur-xl lg:hidden">
            {navItems.slice(0, 5).map((item) => (
              <button
                ...
                className={`relative flex min-h-16 flex-col items-center justify-center gap-1 text-[10px] font-bold ${active === item.label ? "text-primary" : "text-slate-400"}`}
              >
...
function NavItem({
  ...
  return (
    <button
      ...
      className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-semibold transition ${active === item.label ? "bg-primary text-white shadow-lg shadow-primary/20" : "text-white/60 hover:bg-white/5 hover:text-white"}`}
    >
