...
export function AIChatWidget() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessages([...next, { role: "assistant", content: response.ok && typeof data.message === "string" ? data.message : "Please contact our team on WhatsApp and we will help with your vehicle request." }]);
    } catch {
      setMessages([...next, { role: "assistant", content: "The chat service is temporarily unavailable. Please contact our team on WhatsApp." }]);
    } finally {
      setBusy(false);
    }
  }
