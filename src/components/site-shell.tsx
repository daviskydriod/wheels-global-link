...
import logo from "@/assets/awa-logo.png.asset.json";

const nav = [{to:"/",label:"Home"},{to:"/cars",label:"Cars"},{to:"/spare-parts",label:"Spare Parts"},{to:"/about",label:"About Us"},{to:"/why-awa",label:"Why AWA"},{to:"/contact",label:"Contact"}] as const;
...
export function SiteHeader(){
 ...
 return <header className="sticky top-0 z-50 border-b border-border/70 bg-background/95 backdrop-blur-xl">
  <div className="hidden bg-navy py-2 text-primary-foreground md:block"><div className="container-shell flex items-center justify-between text-xs"><span>Guangzhou, China · Global vehicle & parts sourcing</span><div className="flex gap-6"><a href="tel:+8613026895234">+86 130 2689 5234</a><a href={whatsappUrl()}>WhatsApp +971 58 610 6612</a></div></div></div>
  <div className="container-shell grid h-20 grid-cols-[minmax(0,1fr)_auto] items-center gap-4 lg:flex">
   <Link to="/" aria-label="AWA AUTO MALL home" className="min-w-0 lg:mr-auto"><img src={logo.url} alt="AWA AUTO MALL" className="h-14 w-auto max-w-[190px] object-contain" /></Link>
...
export function SiteFooter(){return <footer className="bg-surface-dark text-primary-foreground">
 <div className="container-shell grid gap-10 py-14 md:grid-cols-[1.4fr_1fr_1fr]"><div><img src={logo.url} alt="AWA AUTO MALL" className="h-20 w-auto bg-background p-2"/><p className="mt-5 max-w-sm text-sm text-primary-foreground/65">Cars & Spare Parts sourced from Guangzhou, China for customers around the world.</p></div><div><h2 className="text-lg font-bold uppercase">Navigate</h2><div className="mt-4 grid gap-2 text-sm text-primary-foreground/70">{nav.slice(0,4).concat(nav.slice(5)).map(x=><Link key={x.to} to={x.to} className="hover:text-primary-foreground">{x.label}</Link>)}</div></div><div><h2 className="text-lg font-bold uppercase">Contact</h2><div className="mt-4 grid gap-2 text-sm text-primary-foreground/70"><span>Guangzhou, China</span><a href="tel:+8613026895234">+86 130 2689 5234</a><a href="tel:+233592656665">+233 592 656 665</a><a href={whatsappUrl()}>WhatsApp +971 58 610 6612</a><span>TikTok: AWA Legit Plug</span></div></div></div>
